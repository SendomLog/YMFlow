...
- (void)loadView {
    ...
    YMFlowTask *convlstCGI = [[YMFlowTask alloc] initAll:YMFTaskChannelType_ShortConn andCmdId:kConvLst andCGIUri:@"/mars/getconvlist" andHost:@"www.marsopen.cn"];
    [[YMFNetworkService sharedInstance] startTask:convlstCGI forTaskNotify:self];
}
...
- (int)buf2resp:(NSData*)responseData {
    ...
    YMFLOG_INFO(kYMFlowTAG, @"recv conversation list, size: %lu", (unsigned long)[self->converSations count]);
